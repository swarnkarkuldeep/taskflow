...
const TaskItem: React.FC<TaskItemProps> = ({ task, onToggleComplete, onEdit, onDelete, darkMode }) => {
  ...
  return (
    <div className={`backdrop-blur-xl bg-white/10 rounded-3xl shadow-xl border-l-4 p-8 transition-all duration-300 hover:shadow-2xl hover:bg-white/15 border border-white/20 group ${
      task.completed 
      ...
      <div className="flex items-start gap-6">
        ...
        <button
          ...
          className={`mt-2 w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all duration-300 hover:scale-110 transform ${
            task.completed
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 mb-3">
            {!task.completed && <Sparkles className={`w-5 h-5 ${darkMode ? 'text-purple-300' : 'text-pink-500'} animate-pulse mt-1 flex-shrink-0 transition-colors duration-500`} />}
            <h3 className={`font-bold text-xl leading-tight transition-all duration-300 ${
              task.completed 
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            {/* Priority */}
            <div className="flex items-center gap-2 backdrop-blur-sm bg-white/5 rounded-2xl px-3 py-2">
          ...
          <div className="flex items-center backdrop-blur-sm bg-white/5 rounded-2xl px-4 py-2 inline-flex">
            <Calendar className={`w-4 h-4 mr-2 ${darkMode ? 'text-white/60' : 'text-gray-600/60'} transition-colors duration-300`} />
            <span className={`text-sm font-medium ${darkMode ? 'text-white/60' : 'text-gray-600/60'} transition-colors duration-300`}>Created {formatDate(task.createdAt)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => onEdit(task)}
            className={`p-3 ${darkMode ? 'text-white/70 hover:text-purple-300 hover:bg-purple-500/20' : 'text-gray-600/70 hover:text-pink-500 hover:bg-pink-500/20'} rounded-2xl transition-all duration-300 backdrop-blur-sm border border-white/20 ${darkMode ? 'hover:border-purple-400/50' : 'hover:border-pink-400/50'} hover:scale-110 transform`}
            title="Edit task"
          >
          ...
          <button
            ...
            className={`p-3 rounded-2xl transition-all duration-300 backdrop-blur-sm border hover:scale-110 transform ${
              showDeleteConfirm
      ...
      {showDeleteConfirm && (
        <div className="mt-6 p-4 backdrop-blur-sm bg-red-500/10 border border-red-400/30 rounded-2xl animate-fade-in">
          <p className="text-red-200 font-medium">
            ⚠️ Click the delete button again to confirm removal of this task.
          </p>
        </div>
      )}
    </div>
